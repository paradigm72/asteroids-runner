...
ig.baked=true;ig.module('game.entities.laserShot').requires('impact.entity').defines(function(){EntityLaserShot=ig.Entity.extend({size:{x:10,y:38},collides:ig.Entity.COLLIDES.NONE,maxVel:{x:1000,y:1000},type:ig.Entity.TYPE.A,checkAgainst:ig.Entity.TYPE.B,animSheet:new ig.AnimationSheet('media/projectile_green',10,38),init:function(x,y,settings){this.parent(x,y,settings);this.addAnim('idle',0.1,[0]);this.vel.y=-500;var laserSound=new ig.Sound('media/laser.mp3');laserSound.play();},check:function(entityHit){if(entityHit.name=="wall"){this.kill();}}});});

// lib/game/entities/death.js
ig.baked=true;ig.module('game.entities.death').requires('impact.entity').defines(function(){EntityDeath=ig.Entity.extend({name:"death",size:{x:48,y:48},collides:ig.Entity.COLLIDES.NONE,animSheet:new ig.AnimationSheet('media/death',48,48),init:function(x,y,settings){this.parent(x,y,settings);this.addAnim('idle',0.1,[0,1]);var deathSound=new ig.Sound('media/explosion.mp3');deathSound.play();},update:function(){if(this.anims.idle.loopCount){this.kill();}
this.parent();}});});

// lib/game/entities/ship.js
ig.baked=true;ig.module('game.entities.ship').requires('impact.entity','game.entities.laserShot','game.entities.death').defines(function(){EntityShip=ig.Entity.extend({name:"ship",size:{x:32,y:36},collides:ig.Entity.COLLIDES.ACTIVE,consts:{sideBuffer:4,imageWidth:44,imageHeight:48,laserShotBuffer:4},animSheet:new ig.AnimationSheet('media/ship_1',44,48),init:function(x,y,settings){this.parent(x,y,settings);this.addAnim('idle',0.1,[0,1,2,3,2,1]);this.addAnim('bankLeft',0.1,[4,5,6,7,6,5]);this.addAnim('bankRight',0.1,[8,9,10,11,10,9]);this.maxVel.x=150;},update:function(){if(ig.input.state('left')){this.vel.x=-150;}
else if(ig.input.state('right')){this.vel.x=150;}
else{if(Math.abs(this.vel.x)<=25){this.vel.x=0;}
else{this.vel.x=0.90*this.vel.x;}}
if(ig.input.state('up')){this.vel.y=-100;}
else if(ig.input.state('down')){this.vel.y=100;}
else{this.vel.y=0;}
if(ig.input.pressed('shoot')){ig.game.spawnEntity(EntityLaserShot,this.pos.x+this.consts.laserShotBuffer,this.pos.y);ig.game.spawnEntity(EntityLaserShot,this.pos.x+this.size.x-this.consts.laserShotBuffer,this.pos.y);}
this.pos.y-=1;if(this.pos.x<=0){this.pos.x=this.consts.sideBuffer;this.vel.x=0;}
else if(this.pos.x>=400-this.consts.imageWidth){this.pos.x=400-this.consts.imageWidth-this.consts.sideBuffer;this.vel.x=0;}
if(this.vel.x<0){this.currentAnim=this.anims.bankLeft;}
else if(this.vel.x>0){this.currentAnim=this.anims.bankRight;}
else{this.currentAnim=this.anims.idle;}
ig.game.score+=1;this.parent();},collideWith:function(entity){this.kill();ig.game.spawnEntity(EntityDeath,this.pos.x,this.pos.y);ig.game.gameState=-1;}});});

// lib/game/entities/asteroid.js
ig.baked=true;ig.module('game.entities.asteroid').requires('impact.entity','game.entities.death').defines(function(){EntityAsteroid=ig.Entity.extend({size:{x:48,y:48},collides:ig.Entity.COLLIDES.ACTIVE,type:ig.Entity.TYPE.B,checkAgainst:ig.Entity.TYPE.A,animSheet:new ig.AnimationSheet('media/asteroid',48,48),init:function(x,y,settings){this.parent(x,y,settings);this.vel.y=50+Math.floor((Math.random()*20)+1);var orientation=Math.floor(Math.random()*3);this.addAnim('idle',0.1,[orientation]);},update:function(){this.vel.y+=0.01*(60-this.vel.y);this.parent();},check:function(hitEntity){this.kill();ig.game.spawnEntity(EntityDeath,this.pos.x,this.pos.y);ig.game.score+=100;}});});

// lib/game/entities/wall.js
ig.baked=true;ig.module('game.entities.wall').requires('impact.entity').defines(function(){EntityWall=ig.Entity.extend({name:"wall",size:{x:144,y:48},collides:ig.Entity.COLLIDES.FIXED,animSheet:new ig.AnimationSheet('media/wall',144,48),type:ig.Entity.TYPE.B,checkAgainst:ig.Entity.TYPE.A,init:function(x,y,settings){this.parent(x,y,settings);this.addAnim('idle',0.1,[0]);}});});

// lib/game/entities/tutorial.js
ig.baked=true;ig.module('game.entities.tutorial').requires('impact.entity').defines(function(){EntityTutorial=ig.Entity.extend({type:ig.Entity.TYPE.NONE,checkAgainst:ig.Entity.TYPE.NONE,collides:ig.Entity.COLLIDES.NEVER,size:{x:400,y:75},tooltipCounter:null,animSheet:new ig.AnimationSheet('media/tutorial.png',400,64),init:function(x,y,settings){this.parent(x,y,settings);this.addAnim('idle',1,[0]);this.tooltipCounter=200;},update:function(){this.parent();if(this.tooltipCounter>=0){this.tooltipCounter-=1;this.vel.y=-50;}
else{this.vel.y=-10;}}});});

// lib/game/levels/levelOne.js
ig.baked=true;ig.module('game.levels.levelOne').requires('impact.image','game.entities.asteroid','game.entities.wall','game.entities.tutorial','game.entities.ship').defines(function(){LevelLevelOne={"entities":[{"type":"EntityAsteroid","x":316,"y":540},{"type":"EntityAsteroid","x":332,"y":776},{"type":"EntityAsteroid","x":96,"y":512},{"type":"EntityAsteroid","x":288,"y":392},{"type":"EntityAsteroid","x":88,"y":748},{"type":"EntityWall","x":212,"y":316},{"type":"EntityWall","x":56,"y":96},{"type":"EntityTutorial","x":4,"y":1132},{"type":"EntityShip","x":212,"y":1020}],"layer":[{"name":"background","width":1,"height":3,"linkWithCollision":true,"visible":1,"tilesetName":"media/bg_starfield.jpg","repeat":true,"preRender":false,"distance":"1","tilesize":400,"foreground":false,"data":[[1],[1],[1]]},{"name":"starfield","width":1,"height":3,"linkWithCollision":false,"visible":1,"tilesetName":"media/mg_starfield.png","repeat":true,"preRender":false,"distance":"0.8","tilesize":400,"foreground":false,"data":[[1],[1],[1]]}]};LevelLevelOneResources=[new ig.Image('media/bg_starfield.jpg'),new ig.Image('media/mg_starfield.png')];});

// lib/game/main.js
ig.baked=true;ig.module('game.main').requires('impact.game','impact.font','game.entities.ship','game.entities.asteroid','game.levels.levelOne').defines(function(){MyGame=ig.Game.extend({font:new ig.Font('media/scoreFont.png'),score:0,gameState:0,init:function(){this.loadLevel(LevelLevelOne);ig.input.bind(ig.KEY.LEFT_ARROW,'left');ig.input.bind(ig.KEY.RIGHT_ARROW,'right');ig.input.bind(ig.KEY.DOWN_ARROW,'down');ig.input.bind(ig.KEY.UP_ARROW,'up');ig.input.bind(ig.KEY.SPACE,'shoot');initBackgroundMapScrolling();},update:function(){this.parent();spawnRandomly(EntityAsteroid,'2%');spawnRandomly(EntityWall,'0.2%');scrollBackgroundMap();},draw:function(){this.parent();this.font.draw(this.score,10,10,ig.Font.ALIGN.LEFT);if(this.gameState==-1){this.font.draw("You have died",200,150,ig.Font.ALIGN.CENTER);this.font.draw("Reload to try again",200,190,ig.Font.ALIGN.CENTER);}}});function spawnRandomly(ObjectToSpawn,percentChancePerFrame){var xPos=Math.floor((Math.random()*400)+1);var chanceToSpawn=Math.floor((Math.random()*1000)+1);if(chanceToSpawn>(1000-10*parseFloat(percentChancePerFrame))){ig.game.spawnEntity(ObjectToSpawn,xPos,ig.game.screen.y-192);}}
function initBackgroundMapScrolling(){ig.game.screen.x=0;ig.game.screen.y=800;var myBGMap=ig.game.backgroundMaps[0];myBGMap.setScreenPos(ig.game.screen.x,ig.game.screen.y);var myStarLayer=ig.game.backgroundMaps[1];myStarLayer.setScreenPos(ig.game.screen.x,ig.game.screen.y/myStarLayer.distance);}
function scrollBackgroundMap(){ig.game.screen.y-=1;var myBGMap=ig.game.backgroundMaps[0];myBGMap.setScreenPos(0,myBGMap.scroll.y-1);var myStarLayer=ig.game.backgroundMaps[1];myStarLayer.setScreenPos(0,myStarLayer.scroll.y-1);}
