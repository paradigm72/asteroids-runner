...
MyGame = ig.Game.extend({
    ...
    canSpawnNew: function(ObjectToSpawn) {
        ...
        for (var i = 0; i < walls.length; i++) {
            if (Math.abs(walls[i].pos.y - ig.game.screen.y) < walls[i].size.y) {
                //found something in the top 'row', so quit with false
                canSpawn.asteroid = false;
                canSpawn.wall = false;
                console.log("Try to spawn at screen height: " + ig.game.screen.y +
        ...
        for (var i = 0; i < asteroids.length; i++) {
            if (Math.abs(asteroids[i].pos.y - ig.game.screen.y) < asteroids[i].size.y) {
                //found something in the top 'row', so quit with false
                canSpawn.asteroid = false;
                canSpawn.wall = false;
                console.log("Try to spawn at screen height: " + ig.game.screen.y +
        ...
        for (var i = 0; i < walls.length; i++) {
            if (Math.abs(walls[i].pos.y - ig.game.screen.y) < walls[i].size.y * 3) {
                //found something in the top 'row', so quit with false
