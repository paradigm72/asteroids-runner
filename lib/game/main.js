...
MainGame = ig.Game.extend({
    ...
    shieldBlackEmptyImg: new ig.Image('media/shield/shieldBlackEmpty.png'),
    ...
    init: function() {
        ...
        //Helper object to handle storing/loading high scores
	...
	update: function() {
        ...
        if (this.gameState === this.gameStateEnum.playerDead_ready) {
            ...
            if (ig.input.pressed('replay')) {
                ...
                buttonSound.play();
                this.scoreStorage.saveScoreSorted(this.score);
                this.resetGame();
            }
    ...
    drawShieldMeter: function() {
        ...
        //if there is no shield at all, round the corners
        if (blackStartPos == 0) {
            this.shieldBlackEmptyImg.draw((this.consts.screenWidth - this.shieldMeterImg.width) / 2 +
                blackStartPos,
                this.consts.screenHeight - this.shieldMeterImg.height - 10,
                '','', this.shieldMeterImg.width - blackStartPos - 1);
        }
        //otherwise, show a flat vertical line
        else {
...
TitleScreen = ig.Game.extend({
    ...
    closeButton: new ig.Image('media/buttons/button_green.png'),
    scoreButton: new ig.Image('media/buttons/button_red.png'),
    ...
    init: function() {
        ...
        ig.input.bind(ig.KEY.BACKSPACE, 'scores');
        if (ig.ua.mobile) {
            var beginButtonDims = {
                x: this.LogicalToPixelX(50) - this.closeButton.width / 2,
                y: this.LogicalToPixelY(60),
                width: this.closeButton.width,
                height: this.closeButton.height
            };
            new ig.TouchButton('begin', beginButtonDims.x, beginButtonDims.y, beginButtonDims.width, beginButtonDims.height);

            var scoreButtonDims = {
                x: this.LogicalToPixelX(50) - this.scoreButton.width / 2,
                y: this.LogicalToPixelY(80),
                width: this.scoreButton.width,
                height: this.scoreButton.height
            };
            new ig.TouchButton('scores', scoreButtonDims.x, scoreButtonDims.y, scoreButtonDims.width, scoreButtonDims.height);
        }
    },
    ...
    draw: function() {
        ...
        var beginButtonY = this.LogicalToPixelY(60);
        this.closeButton.draw(this.LogicalToPixelX(50) - this.closeButton.width / 2,
                              beginButtonY);
        this.titleFont.draw("PLAY!", this.LogicalToPixelX(50), beginButtonY,
                       ig.Font.ALIGN.CENTER);

        var scoresButtonY = this.LogicalToPixelY(80);
        this.scoreButton.draw(this.LogicalToPixelX(50) - this.scoreButton.width / 2,
                              scoresButtonY);
        this.titleFont.draw("SCORES", this.LogicalToPixelX(50), scoresButtonY,
            ig.Font.ALIGN.CENTER);

//        if (this.scoreStorage.getTopScoresSorted(1)[0] > 0) {
//            this.scoreFont.draw("High Score: " + this.scoreStorage.getTopScoresSorted(1)[0],
//                this.LogicalToPixelX(50), this.LogicalToPixelY(50),
//                ig.Font.ALIGN.CENTER);
//        }
    ...
    update: function() {
        ...
        if (ig.input.state('scores')) {
            ig.input.unbindAll();
            var buttonSound = ig.soundManager.get('media/sounds/menu_button.mp3');
            buttonSound.play();
            ig.system.setGame(ScoreScreen);
        }
    }

});

ScoreScreen = ig.Game.extend({
    // Load a font
    titleFont: new ig.Font( 'media/titleFont.png' ),
    scoreFont: new ig.Font( 'media/scoreFont.png' ),

    closeButton: new ig.Image('media/buttons/button_red.png'),
    ...
    init: function() {
        ...
        ig.input.bind(ig.KEY.BACKSPACE, 'close');
        ig.input.bind(ig.KEY.SPACE, 'close');
        ig.input.bind(ig.KEY.ENTER, 'close');
        if (ig.ua.mobile) {
            var buttonDims = {
                x: this.LogicalToPixelX(50) - this.closeButton.width / 2,
                y: this.LogicalToPixelY(90),
                width: this.closeButton.width,
                height: this.closeButton.height
            };
            new ig.TouchButton('close', buttonDims.x, buttonDims.y, buttonDims.width, buttonDims.height);
        }
    },
    ...
    draw: function() {
        ...
        this.titleFont.draw("High Scores", this.LogicalToPixelX(50), this.LogicalToPixelY(10), ig.Font.ALIGN.CENTER);

        //draw the score list
        var scoresList = this.scoreStorage.getTopScoresSorted(10);
        var timesList = this.scoreStorage.getTimesSorted(10);
        var scoreStartHeight = this.LogicalToPixelY(20);
        for (i = 0; i < Math.min(10, this.scoreStorage.getScoreCount()); i++) {
            if (scoresList[i].toString() != null) {
                this.scoreFont.draw(scoresList[i], this.LogicalToPixelX(5), scoreStartHeight + i * 24);
            }
            this.scoreFont.draw(timesList[i], this.LogicalToPixelX(30), scoreStartHeight + i * 24);
        }

        var closeButtonY = this.LogicalToPixelY(90);
        this.closeButton.draw(this.LogicalToPixelX(50) - this.closeButton.width / 2,
            closeButtonY);
        this.titleFont.draw("CLOSE", this.LogicalToPixelX(50), closeButtonY,
            ig.Font.ALIGN.CENTER);

    },

    update: function() {
        if (ig.input.state('close')) {
            ig.input.unbindAll();
            var buttonSound = ig.soundManager.get('media/sounds/menu_button.mp3');
            buttonSound.play();
