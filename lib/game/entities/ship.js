...
EntityShip = ig.Entity.extend({
	...
	init: function(x, y, settings) {
		...
		this.animSheet = new ig.AnimationSheet( 'media/ship_1', this.consts.imageWidth, this.consts.imageHeight);		
    ...
    processAccelerometerInput: function() {
        //movement left and right
        if (ig.input.accel.x > 0) {
            this.accel.x = +750;
        }
        else if (ig.input.accel.x < 0) {
            this.accel.x = -750;
        }
        else {
            //right now, we actually hit the else block of the keyboard input processor...
        }

        //movement up and down
        if (ig.input.accel.y > 0) {
            this.vel.y = -10;
        }
        else if (ig.input.accel.y < 0) {
            this.vel.y = +10;
        }
        else {
            if (Math.abs(this.vel.y) <= 25) {
        ...
        //shoot is handled by the keyboard input right now (touch binds to 'shoot')
    },
