...
EntityShip = ig.Entity.extend({
	...
	init: function(x, y, settings) {
		...
		this.animSheet = new ig.AnimationSheet( 'media/ship/ship_1.png', this.consts.imageWidth, this.consts.imageHeight);
    ...
    toggleShield: function(newState) {
        this.shieldUp = newState;
    },
	...
	//if we collide with an object (wall), the player dies
    ...
    killMe: function() {
        ...
        this.kill();
        ig.game.spawnEntity(EntityShipExplosion, this.pos.x + this.size.x / 2,
    ...
    processAccelerometerInput: function() {
        //movement left and right
        if (ig.input.accel.x < -0.07) {
            //max velocity is 150, so we reach that very quickly
            this.accel.x = 2500 *  ig.input.accel.x;
        }
        else if (ig.input.accel.x > 0.07) {
            this.accel.x = 2500 * ig.input.accel.x;
        }
        else {
            //immediately stop accelerating
